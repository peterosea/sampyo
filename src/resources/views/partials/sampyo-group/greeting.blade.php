
<main class="sampyoGroup__page page__layout">
<div class="container xl:max-w-screen-xl mx-auto">
  <div class="page__level-title mb-[30px] sm:mb-20">
    고객과 사회의 지속가능한 발전에 힘이 되는 기업이 되겠습니다
  </div>
  <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-12 gap-y-7 sm:gap-y-28 mb-[30px] sm:mb-20">
    <div class="prose">
      <p class="mb-10">
        삼표그룹은 1966년 창립 이래 ‘개척 정신’을 바탕으로 우리나라 기간 산업의 한 축으로서 굳건히 자리매김해왔습니다. 지난 반세기 이상 지속적인 변화와 도전을 통해 고객과 사회 및 국가 발전에 이바지하며 성장을 이어왔습니다.  
      </p>
      <p class="mb-10">
        급속한 경제 발전에 따른 대규모의 건설 수요에 대처하기 위해 끊임없는 기술 혁신과 고품질의 제품을 안정적으로 공급하여 고객 및 파트너와 함께 대한민국의 인프라 구축에 전력을 다했습니다. 
      </p>
      <p class="mb-10">
        인프라 건설의 기초가 되는 건설 소재 사업의 수직 계열화를 완성함으로써, 국내 유일의 건설 소재 종합 솔루션 공급자(Total Solution Provider)로 자리매김했습니다. 나아가 철도 궤도 및 물류 분야에서도 탄탄한 입지를 구축했습니다. 자원 재활용을 통한 자원부족 해결과 환경보존을 실천하는 환경자원 사업을 확대해왔습니다. 
      </p>
    </div>
    <div class="m_sm:order-first">
      @include('partials.img', ['ex' => 'jpg', 'name' => 'chairman_message-1'])
    </div>
    <div>
      @include('partials.img', ['ex' => 'jpg', 'name' => 'chairman_message-2'])
    </div>
    <div class="prose">
      <p class="mb-10">
        이제 세상은 새로운 도전 과제에 직면했습니다. 기후 위기를 해결하기 위해 이산화 탄소를 줄이고, 순환자원의 활용을 늘려야 합니다.
      </p>
      <p class="mb-10">
        삼표그룹은 대한민국의 지속 가능한 발전을 위해 사업 전 영역에서 탄소제로 2050을 목표로 탄소 감축을 이루겠습니다.
      </p>
      <p class="mb-10">
        <b>
          천연광산을 필두로 산림자원 개발과 새로운 가치를 캐내는 도시광산까지 사업을 다각화하고 지구환경 보호를 위한 환경자원 분야의 성장에 사명감을 가집니다.<br/>
          기술과 혁신을 통해 물류 등 사회의 다양한 요구를 충족시키기 위해 노력합니다. 또한 협력사와 지역사회는 물론 모든 이해관계자와 동반성장을 통한 공동체의 발전을 추구합니다. 
        </b>
      </p>
    </div>
  </div>
  <p class="text-2xl mb-11" style="line-height: 1.67">
    <span class="pointUnderLine" style="display: inline; --bs: var(--RGB-atmosphere); --h: 0.6em; --op: 0.2">
      삼표그룹은 끝없는 개척과 도전의 정신으로 고객의 성공과 대한민국의 지속가능한 발전에 기여하겠습니다.
    </span><br/><br class="sm:hidden" />
    <span>
      감사합니다.
    </span>
  </p>
  {{-- <p class="text-lg font-bold">삼표그룹 회장 | 정도원 </p> --}}
</div>
</main>