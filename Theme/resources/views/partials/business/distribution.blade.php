<main class="page__layout">
<div class="container xl:max-w-screen-xl mx-auto">
  <section class="mb-[70px] sm:mb-32">
    <div class="page__level-title mb-[30px] sm:mb-16 pb-1.5">
      선진화된 물류시스템으로 신속하고 정확한 <br class="hidden sm:block" />
      물류서비스를 제공합니다.
    </div>
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-[50px] gap-y-[70px]">
      <div class="prose">
        <p>
          삼표는 최고의 건설복합물류서비스 전문기업으로 1966년 창업 이래 끊임없는 개발과 혁신으로 전국에 물류 네트워크를 구축하고, 선진화된 물류시스템을 기반으로 전 운송과정을 관리하는 물류서비스를 제공합니다.
        </p>
      </div>
      <div class="prose">
        또한, 기업 경쟁력의 핵심인 물류, 운송 구조를 효율적으로 개선함으로써 기업의 물류비용을 줄이고 물류 생산성을 향상시켜, 기업의 이익 증대를 최우선 목표로 하고 있습니다. 앞으로도 오랜 기간 축적해온 노하우를 바탕으로 고객의 다양한 니즈 충족과 효율적인 물류 운영으로 대한민국 건설복합물류 대표기업으로 자리매김하겠습니다.
      </div>
      <div>
        <div class="coverbox-label">
          @include('partials.img', ['ex' => 'jpg', 'name' => 'business-distribution-0'])
          <span class="label text-white">최적화된 물류인프라</span>
        </div>
      </div>
      <div>
        <div class="coverbox-label">
          @include('partials.img', ['ex' => 'jpg', 'name' => 'business-distribution-1'])
          <span class="label text-white">선진화된 물류시스템</span>
        </div>
      </div>
    </div>
  </section>
  <section class="mb-[130px]">
    <div class="coverbox-full left pt-[30px] sm:py-28">
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-[50px] max:sm:mb-[30px]">
        <div>
          <div class="page__level-title mb-7">
            고객 맞춤형 물류인프라
          </div>
          <div class="prose mb-[30px] sm:mb-[80px]">
            <p>
              수도권을 중심으로 전국에 걸친 거점 시설을 확보하고, 다양하고 안정적인 차량확보로 고객이 원하는 최적의 서비스를 제공하고 있습니다.
            </p>
          </div>
          <div class="mb-7">
            <div class="font-bold mb-3 text-[20px]">
              <span class="pointUnderLine">
                물류 거점 및 위치도
              </span>
            </div>
            <div class="prose">
              <p>
                전국적으로 육송거점 26개소, 해송거점 13개소, 철송거점 3개소를 운영하여 전국적인 물류 네트워크를 구축하고 있습니다.
              </p>
            </div>
          </div>
          <div>
            <ul class="mb-[24px]">
              <li class="li__single-s1">육송거점 : 26개소 (양주, 화성, 천안 등)</li>
              <li class="li__single-s1 atmosphere">해송거점 : 13개소 (삼척, 부산, 제주 등)</li>
              <li class="li__single-s1 bermudagrass">철송거점 : 3개소 (부곡, 대전, 도안)</li>
            </ul>
            <div class="flex gap-x-[20px] w-full">
              <div class="flex gap-x-[5px] items-center">
                @include('partials.img', ['ex' => 'png', 'name' => 'business-distribution-3'])
                <span>
                  골재석산
                </span>
              </div>
              <div class="flex gap-x-[5px] items-center">
                @include('partials.img', ['ex' => 'png', 'name' => 'business-distribution-4'])
                <span>
                  발파석
                </span>
              </div>
            </div>
          </div>
        </div>
        <div class="relative max:sm:transform max:sm:translate-y-[100px]">
          @include('partials.img', ['ex' => 'png', 'name' => 'business-distribution-2', 'class' => 'sm:absolute w-full'])
        </div>
      </div>
    </div>
  </section>
  <section class="mb-[70px] sm:mb-32">
    <div class="page__level-title mb-[30px] sm:mb-[40px]">
      운용 차량
    </div>
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-[50px] mb-[30px] sm:mb-[70px]">
      <div class="prose">
        <p>
          직영차(자차/지입/직계약) 기준 덤프트럭 400대, BCT 100대를 비롯하여 총 600대가량의 차량을 확보하여 안정적인 운송구조를 구축하고, 200여개 운송 협력업체와의 계약을 통해 차량 수급여건에 따른 효율적인 차량운영을 하고 있습니다. 
        </p>
      </div>
      <div class="prose">
        <p>
          이러한 전국 물류 네트워크을 토대로 발전소, 제철소 등 대형 외부거래처와의 운송 수주를 확대하고, 연계운송을 통한 물류 효율화로 화주 원가 절감에 기여합니다. <br/>
          IT 기술을 활용한 차량 관제 시스템으로 물류 네트워크를 구축함으로써 국내 BCT 물류사업을 선도하고 있습니다.
        </p>
      </div>
    </div>
    <div class="border-t border-b border-squant py-[60px] mb-[70px] sm:mb-[110px]">
      <div id="swiper" class="swiper-container">
        <div class="swiper-wrapper">
          <div class="swiper-slide">
            <div class="card__s4 w-auto">
              <div class="card__s4-thumbnail border w-[210px] h-[210px]">
                @include('partials.img', ['ex' => 'jpg', 'name' => 'business-distribution-5'])
              </div>
              <div class="card__s4-body">
                <div class="card__s4-title text-[16px] text-center font-bold">
                  벌크트레일러<br/>
                  (Bulk Cement Trailer)
                </div>
              </div>
            </div>
          </div>
          <div class="swiper-slide">
            <div class="card__s4 w-auto">
              <div class="card__s4-thumbnail border w-[210px] h-[210px]">
                @include('partials.img', ['ex' => 'jpg', 'name' => 'business-distribution-6'])
              </div>
              <div class="card__s4-body">
                <div class="card__s4-title text-[16px] text-center font-bold">
                  M/T(믹서트럭)부문
                </div>
              </div>
            </div>
          </div>
          <div class="swiper-slide">
            <div class="card__s4 w-auto">
              <div class="card__s4-thumbnail border w-[210px] h-[210px]">
                @include('partials.img', ['ex' => 'jpg', 'name' => 'business-distribution-7'])
              </div>
              <div class="card__s4-body">
                <div class="card__s4-title text-[16px] text-center font-bold">
                  D/T(덤프트럭) 부문
                </div>
              </div>
            </div>
          </div>
          <div class="swiper-slide">
            <div class="card__s4 w-auto">
              <div class="card__s4-thumbnail border w-[210px] h-[210px]">
                @include('partials.img', ['ex' => 'jpg', 'name' => 'business-distribution-8'])
              </div>
              <div class="card__s4-body">
                <div class="card__s4-title text-[16px] text-center font-bold">
                  F/C(평판트레일러) 부문
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-[50px] gap-y-[30px] mb-[30px] sm:mb-[70px]">
      @include('partials.img', ['ex' => 'jpg', 'name' => 'business-distribution-9', 'class' => 'order-1 sm:order-none'])
      @include('partials.img', ['ex' => 'jpg', 'name' => 'business-distribution-10', 'class' => 'order-3 sm:order-none'])
      <div class="prose order-2 sm:order-none">
        <p>
          해안지역에 생산공장을 운영하고 있는 삼표시멘트는 전용선박 11척, 일반 화물선박 3척을 활용하여 국내 시멘트 해상 운송 시장을 선도하고 있습니다. 또한, 제철소에서 생산되는 슬래그 등을 선박을 이용해 전국에 신속하게 공급하며, 건설기초재료인 바닷모래를 채취해 수도권에 안정적인 모래 공급을 하고 있습니다.
        </p>
      </div>
      <div class="prose order-4 sm:order-none">
        <p>
          당사는 230량의 시멘트 화차와 기관차 2대를 보유하고 있으며, 철도를 활용하여 내륙 공장인 부곡, 대전, 도안으로 시멘트를 수송하고 있습니다. 친환경 수단이면서 적기에 납품이 가능한 철도 운송은 삼표시멘트의 중요한 운송수단으로 자리 잡고 있습니다.
        </p>
      </div>
    </div>
  </section>
  <section>
    <div class="page__level-title mb-[30px] sm:mb-[40px]">
      첨단기술의 물류시스템
    </div>
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-[50px] gap-y-[30px] sm:gap-y-[60px] mb-[30px] sm:mb-[70px]">
      <div class="prose">
        <p>
          삼표는 전국 운송 네트워크와 인프라를 비롯해 다양한 건자재 화물에 최적화된 차량 및 장비로 건자재 물류 서비스를 제공하고 있습니다. 최근에는 운송 시장 변화에 대한 선제적 대응과 서비스 질 향상을 위해 IT 기반 관리 체계 구축 및 전문 인력 역량 강화 추진에 힘쓰고 있습니다.
        </p>
      </div>
      <div class="prose">
        <p>
          또한 전통적인 물류시장 구조에서 발전하여 GPS, Mobile, RFID를 활용해 IT와 접목한 선진화된 물류 시스템을 운영하고 있으며, 전국 각 거점에 출하자동화 시스템을 구축하고, 통합관리시스템을 운영함으로써 제품 입/출고의 자동화를 꾀하고, 물류 시장의 주체들을 하나의 네트워크로 연결하는 물류 플랫폼을 개발하여 변화하는 물류시장의 선두기업으로 발돋음하고 있습니다.
        </p>
      </div>
      <div class="sm:col-span-2">
        @include('partials.img', ['ex' => 'jpg', 'name' => 'business-distribution-11'])
      </div>
      @foreach ([
        [
          'title' => <<<EOD
          삼표 모바일 시스템
EOD,
          'content' => <<<EOD
            삼표는 스마트폰을 이용해 고객 주문에 대해 실시간으로 정보를 제공하고 있습니다. 삼표의 모바일 시스템은 타설 현황, 대기차량 현황, 이동중인 차량 정보, 출하 현황 등을 제공해 현장에 있지 않아도 현장을 볼 수 있는 눈이 되고 있습니다. 2021년에는 덤프트럭 관련 업종에 종사하는 모든 분을 대상으로 하는 물류 서비스 플랫폼 3종, ‘덤프로’, ‘덤프로 업’, ‘덤프로 톡’을 출시하였습니다.
EOD
        ],
        [
          'title' => <<<EOD
          자동 출하시스템
EOD,
          'content' => <<<EOD
          삼표는 차량 배차부터 입출고, 납품서 발행, 제품 상ㆍ하차에 이르는 출하 전 과정을 무인 자동화로 운영하는 첨단 시스템을 업계 최초로 갖추고 있습니다.<br/>
          자동출하시스템 도입으로 출하 시간이 약 20% 단축되었으며 고객이 원하는 시간에 맞춰 보다 신속하게 제품을 공급할 수 있습니다.
EOD
        ],
        [
          'title' => <<<EOD
          RFID 기반 물류관제시스템
EOD,
          'content' => <<<EOD
          RFID 기반의 물류관제 시스템을 통하여 산지부터 출하지까지의 물류를 실시간으로 추적 관리함으로써 신속 정확한 물류 서비스를 제공하고, 전국의 물류 거점을 확보, 차량 운영 효율화를 통해 물류비를 획기적으로 절감하고 있습니다.
EOD
        ],
        [
          'title' => <<<EOD
          전자전표
EOD,
          'content' => <<<EOD
          실물 전표를 전자전표로 대체하여 운송관리 업무를 효율화하였고, 자동정산 시스템을 연계하여 On-Line 운송관리 체계를 구축하였습니다.
EOD
        ],
      ] as $item)
        <div class="prose">
          <h3 class="underline power mb-[30px] font-bold text-[24px]">
            {!! $item['title'] !!}
          </h3>
          <p>
            {!! $item['content'] !!}
          </p>
        </div>
      @endforeach
    </div>
  </section>
</div>
</main>